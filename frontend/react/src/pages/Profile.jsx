...
const Profile = () => {
  ...
  useEffect(() => {
    const fetchProfileData = async () => {
      try {
        const response = await axios.get(`http://127.0.0.1:8000/api/profile/${username}/`); // Ensure the URL is correct
    ...
    <>
      <AuthRedirect>
          ...
          {/* Profile Header */}
            <div className="relative bg-white rounded-xl shadow-md p-6 text-center">
              ...
              <div className="absolute top-4 left-4">
                <button
                  onClick={() => window.history.back()} // Add functionality to navigate back
        ...
        <div className="text-gray-500 text-sm sm:text-base mt-2 md:mt-1 flex items-center justify-center">
          <Icon name="location" className="h-[15px] w-[15px] sm:h-5 sm:w-5 mt-1 mr-1" />
        ...
        <Button
          ...
          label={isRequestLoading ? 'Sending...' : 'Request Sent'}
          icon={<Icon name="request" className="h-5 w-5" />}
        ...
        />
              ...
              <div className="bg-white rounded-xl shadow-md p-6 mt-6">
                <h2 className="text-lg sm:text-xl font-semiboldfont-inter text-secondary-700">Room Details</h2>
                <table className="w-full mt-2 text-sm sm:text-base text-gray-600">
                  <tbody>
                    <tr>
